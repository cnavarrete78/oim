...
BEGIN		
	...
	BEGIN
		...
		BEGIN	
			DECLARE @FECHA_SISTEMA DATETIME
			SET @FECHA_SISTEMA = GETDATE()

			INSERT INTO [RYR_COMUNITARIO].TB_PLAN_ACCION_TRASLADO_PERSONA_NO_TRASLADA  (ID_COMUNIDAD,ID_PLAN_ACCION_TRASLADO, ID_PERSONA,FECHA_SISTEMA,MOTIVO)	 VALUES (@ID_COMUNIDAD,@ID_PLAN_ACCION_TRASLADO, @ID_PERSONA,@FECHA_SISTEMA,@MOTIVO)		
		END
		ELSE
		BEGIN
			UPDATE [RYR_COMUNITARIO].TB_PLAN_ACCION_TRASLADO_PERSONA_NO_TRASLADA  SET MOTIVO =@MOTIVO WHERE ID_COMUNIDAD = @ID_COMUNIDAD AND ID_PLAN_ACCION_TRASLADO= @ID_PLAN_ACCION_TRASLADO AND ID_PERSONA =@ID_PERSONA		
		END
	END
	ELSE
	BEGIN
		DELETE [RYR_COMUNITARIO].TB_PLAN_ACCION_TRASLADO_PERSONA_NO_TRASLADA  WHERE ID_COMUNIDAD = @ID_COMUNIDAD AND ID_PLAN_ACCION_TRASLADO= @ID_PLAN_ACCION_TRASLADO AND ID_PERSONA =@ID_PERSONA			
	END
END
go
...
AS
BEGIN		
	select HP.ID_PERSONA, CH.ID_HOGAR AS HOGAR, ISNULL(P.PER_CNOMBRE1, '') + ' ' + ISNULL( P.PER_CNOMBRE2, '') + ' ' + ISNULL(P.PER_CAPELLIDO1, '') + ' ' + ISNULL(P.PER_CAPELLIDO2, '') AS 'PERSONA', [TIPO_IDENTIFICACION]+ ' ' + P.PER_CCEDULA AS DOCUMENTO
	from [RYR_COMUNITARIO].[TB_CARACTERIZACION_COMUNIDAD] AS C
	JOIN [RYR_COMUNITARIO].[TB_CARACTERIZACION_COMUNIDAD_DETALLE] AS D ON C.ID_CARACTERIZACION_COMUNIDAD = D.ID_CARACTERIZACION_COMUNIDAD
	JOIN [RYR_COMUNITARIO].[TB_CARACTERIZACION_COMUNIDAD_HOGAR] AS H ON D.ID_CARACTERIZACION_COMUNIDAD = H.ID_CARACTERIZACION_COMUNIDAD
	JOIN [RYR_COMUNITARIO].[TB_CARACTERIZACION_COMUNIDAD_HOGAR_PERSONA] AS HP ON H.ID_CARACTERIZACION_COMUNIDAD_HOGAR = HP.ID_CARACTERIZACION_COMUNIDAD_HOGAR
	...
	WHERE NT.ID_PLAN_ACCION_TRASLADO =@ID_PLAN_ACCION_TRASLADO
