...
public partial class Ruta_RyR_Comunitario : System.Web.UI.Page
{
            ...
            if (e.CommandName == "Actualizar")
            {
                ...
                switch (Convert.ToInt32(ViewState["id_nombre_actividad"]))
                {
                    case 298://Fase 2  - Ficha de Caracterización
                        ProcesarFicha();
                        break;
                    case 301://si es Fase 5 - desarrollo del traslado
                        procesaPlanTraslado();
                        break;
                    case 302://Fase 6 - Plan de retorno y reubicaciòn
                        ProcesarPlanRyR();
                        break;
                    case 304:// si es Fase 8 - cierre / balance del acompañamiento
                        procesaBalanceTraslado();
                        break;
                    default:
    ...
    protected void btn_exportar_comunidad_Click(object sender, EventArgs e)
    {
        this.gv_personas_exportar.DataSource = Ficha.Personas;
        this.gv_personas_exportar.DataBind();
        Up_personas_exportar.Update();
        ScriptManager.RegisterStartupScript(Page, Page.GetType(), "myModalExportarPoblacion", "$('#myModalExportarPoblacion').modal();", true);
    }
    ...
    {
        if (gv_PersonasFicha.Rows.Count > 0)
        {
        ...
        if (gv_PersonasFicha.Rows.Count == 1)
        {
        }
    }
    protected void gv_personas_exportar_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
        }
    ...
    protected void gv_personas_exportar_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
        }
    }
    protected void gv_personas_exportar_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        Up_personas_exportar.Update();
    }
    protected void gv_personas_exportar_PreRender(object sender, EventArgs e)
    {
        if (gv_personas_exportar.Rows.Count > 0)
        {
            gv_personas_exportar.HeaderRow.Cells[0].Attributes["data-priority"] = "1";
            gv_personas_exportar.HeaderRow.Cells[1].Attributes["data-priority"] = "2";
            gv_personas_exportar.HeaderRow.Cells[2].Attributes["data-priority"] = "2";
            gv_personas_exportar.HeaderRow.Cells[3].Attributes["data-priority"] = "1";
            gv_personas_exportar.HeaderRow.Cells[4].Attributes["data-priority"] = "2";
            gv_personas_exportar.UseAccessibleHeader = true;
            gv_personas_exportar.HeaderRow.TableSection = TableRowSection.TableHeader;
            gv_personas_exportar.FooterRow.TableSection = TableRowSection.TableFooter;
        }
        if (gv_personas_exportar.Rows.Count == 1)
        {
        }
    }
    #endregion
    ...
    protected void btn_exportar_comunidad_plan_Click(object sender, EventArgs e)
    {
        this.gv_personas_exportar.DataSource = PlanRyR.Personas;
        this.gv_personas_exportar.DataBind();
        Up_personas_exportar.Update();
        ScriptManager.RegisterStartupScript(Page, Page.GetType(), "myModalExportarPoblacion", "$('#myModalExportarPoblacion').modal();", true);
    }
    ...
    protected void gv_plan_ryr_necesidad_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName == "EditarDetalleNecesidadPlanRyR")
            {
                GridViewRow gvRow = (GridViewRow)(((LinkButton)e.CommandSource).NamingContainer);
                int idNecesidad = Convert.ToInt32(gv_plan_ryr_necesidad.DataKeys[gvRow.DataItemIndex].Values[0]);
                foreach (DataRow necesidad in PlanRyR.Necesidades.Tables[0].Rows)
                    if (necesidad["ID_NECESIDAD"].Equals(idNecesidad))
    ...
    protected void btn_grabar_necesidades_Click(object sender, EventArgs e)
    {
        try
        {
            PlanRyR.Necesidad = Convert.ToInt32(this.ID_NECESIDAD.Text);
            PlanRyR.Acciones = this.ACCIONES.Text;
    ...
    #region DESARROLLO PARA EL TAB DE PLAN DE TRASLADO

    private void procesaPlanTraslado()
    {
        ...
        Lista.L_D_Territorial(ref LD_DireccionTerritorialProfesionalEncargado);
        Lista.L_D_Tipo_Evidencia(ref LD_TipoEvidencia);
        GetPlanTraslado();
    ...
    private void procesaBalanceTraslado()
    {
        ...
        if (existeplan)
        {
            ...
            Lista.L_D_Territorial(ref LD_DireccionTerritorialProfesionalListadoBalance);
        ...
        {
            Mensajes("No se tiene un plan de traslado. Por favor diligenciarlo en la fase 5.", 0);
        }
    }
    ...
    {
        Lista.L_D_Departamentos(ref LD_Departamento_Salida);
        Lista.L_D_Departamentos(ref LD_Departamento_Llegada);
        Lista.L_D_Departamentos(ref LD_DepartamentoAlistamiento);
    }
    ...
    {
        Lista.L_D_Municipios(ref LD_Municipio_Salida, idDepartamento);
    }
    public void LlenarComboMunicipioLlegada(int idDepartamento)
    {
        Lista.L_D_Municipios(ref LD_Municipio_Llegada, idDepartamento);
    }
    public void LlenarComboMunicipioAlistamiento(int idDepartamento)
    {
        Lista.L_D_Municipios(ref LD_MunicipioAlistamiento, idDepartamento);
    }
    public void LlenarComboEntidadesRutaComunitaria()
    {
        Lista.L_D_Entidad(ref LD_Entidad);
        Lista.L_D_Entidad(ref LD_EntidadRegistra);
        Lista.L_D_Entidad(ref LD_EntidadProfesionalRegistra);
        Lista.L_D_Entidad(ref LD_EntidadCaracterizacion);
    }
    ...
    protected void gv_personas_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        try
        {
            if (e.CommandName == "Seleccionar")
            {
                ...
                int idComunidad =  Convert.ToInt32(ViewState["ID_RYR_COMUNIDAD"]);
                string motivo = txMotivoNoTraslado.Text;
                int idUsuario = Convert.ToInt32(Session["id_usuario"]);
                bool exitoso = FachadaPersistencia.getInstancia().LD_Modificar_Persona_trasladar_plan_acción_traslado_ruta_comunitaria(idPlan, idComunidad, idPersona, seTraslada, motivo, idUsuario);
                if (exitoso)
                {
    ...
    protected void btn_guardar_plan_Accion_traslado_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            int idComuniad = Convert.ToInt32(ViewState["ID_RYR_COMUNIDAD"]); 
        ...
        {
            Mensajes("Error al guardar la información." + ex.Message, 0);
        }
    }
    ...
    protected void btn_agregar_Datos_Alistamiento_Traslado_Click(object sender, EventArgs e)
    {
        ...
        {
            Mensajes("Error al guardar información del alistamiento logístico." + ex, 0);
        }
    }
    ...
    //METODOS QUE ELIMINAN INFORMACION DE PLAN DE TRASLADO
            ...
            if ((FileUploadEvidencia.FileName != "") && (FileUploadEvidencia.PostedFile != null))
            {
                if (tamano < 10485760)
                {
                        ...
                        )
                    {
                        ...
                        string path = System.Configuration.ConfigurationManager.AppSettings["Archivos"] + this.archivo_filesystem.Value;
                        // TODO: MODIFICAR
                        string FileName = ViewState["ID_RYR_COMUNIDAD"].ToString() + "_" + (DateTime.Now.ToString("dd-MM-yyyy hh-mm-ss").Replace(" ", "H")) + extension;
    ...
    #region DESARROLLO PARA EL TAB DEL BALANCE
    ...
    protected void gv_listado_personas_que_se_acompanan_PreRender(object sender, EventArgs e)
    {
        if (gv.Rows.Count > 0)
        {
            gv_listado_personas_que_se_acompanan.HeaderRow.Cells[0].Attributes["data-priority"] = "1";
            gv_listado_personas_que_se_acompanan.HeaderRow.Cells[1].Attributes["data-priority"] = "2";
            gv_listado_personas_que_se_acompanan.HeaderRow.Cells[2].Attributes["data-priority"] = "2";
            gv_listado_personas_que_se_acompanan.HeaderRow.Cells[3].Attributes["data-priority"] = "1";
    ...
    public bool GetPlanTrasladoParaBalance()
    {
        DataSet dsPT = new DataSet();
        int idComunidad = Convert.ToInt32(ViewState["ID_RYR_COMUNIDAD"]);
        dsPT = FachadaPersistencia.getInstancia().GetPlanTrasladoPorComunidad(idComunidad);
    ...
    protected void btn_guardar_datosProfesionalListado_balance_Click(object sender, EventArgs e)
    {
        try
        {
            int idComunidad = Convert.ToInt32(ViewState["ID_RYR_COMUNIDAD"]);
            int idPlan = Convert.ToInt32(ViewState["idPlanAccionTraslado"]);
            int idDt = Convert.ToInt32(LD_DireccionTerritorialProfesionalListadoBalance.SelectedValue);
            string profesional = txProfesionalListadoBalance.Text;
    ...
    protected void mostar_reportes_comunitario_Click(object sender, EventArgs e)
    {
        ...
        m_Balance.Visible = false;
        balance_ryr.Visible = false;
        PanelPT.Visible = false;
        UP_DatosEvento.Visible = false;
        LimpiaFormulario(this);

        //carga informacion del reporte
        panelReportes.Visible = true;
        UpdatePanelReportes.Update();
        Lista.L_D_Reportes(ref LD_Reporte);
    }
