...
public partial class Descargar_evidencia : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            try
            {
                ...
                else if (Request.QueryString["ID_BALANCE"] != null)
                {
                    ...
                    FileStream Archivo = new FileStream(rutaCompleta, FileMode.Open, FileAccess.Read);
                    byte[] Contenido;
                    Contenido = new byte[Archivo.Length];
                    Archivo.Read(Contenido, 0, (int)Archivo.Length);
                    Response.ClearContent(); //Borra todo el contenido del buffer
                    ...
                    {
                        Response.AddHeader("content-disposition", " filename=" + nombre_archivo + extension); //attachment;
                    }
                    else
                    {
                        Response.AddHeader("content-disposition", "attachment; filename=" + nombre_archivo + extension); //attachment;
                    }
                    Response.BinaryWrite(Contenido); //Escribe datos binarios en el flujo de salida HTTP y por tanto, se muestra o descarga el archivo
                    Archivo.Close();
                    Contenido = null;
